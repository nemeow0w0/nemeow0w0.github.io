**Qr code Generator**

ที่มา 
เป็นโปรเจคแรกที่ได้ทำตอนฝึกงาน พี่เลี้ยงให้ทำเพื่อดูความเข้าใจในการทำเว็บไซต์และการใช้งาน libary QRCODE โดยปัญหาคือ เว็บไซต์ **Qr code Generator** โดยทั่วไปมักมีโฆษนา ต้องการที่ไม่มีโฆษณาจึงได้ให้ลองทำ**Qr code Generator** ขึ้นมา

---

**วัตถุประสงค์**
 - ฝึกการเขียนเว็บไซต์
 - ฝึกการเขียนโปรแกรม
 - ฝึกการใช้งาน Libary QR Code 
 - ทำความเข้าใจการเผยแพร่เว็บไซต์ ด้วย gitgubpage 

---
**เทคโนโลยีที่ใช้**

### Frontend
  - React.js (Vite)
  - JavaScript
  - CSS / Tailwind CSS (พื้นฐาน)

### Tools
  - github
 
---
**ฟีเจอ**
- เพิ่ม / เปลี่ยนโลโก้ ตรงกลาง QR Code ได้
- รับรองรูบแบบข้อมูลหลายประเภท (ข้อความ Url เบอร์มือถือ)

---
 **สิ่งที่ได้เรียนรู้**
 - การใช้งาน libary QR Code
 - ทำความเข้าใจการใช้งาน state ใน React
 - การ Deploy บน Github Page


